...
func GetActiveGraph(ctx context.Context) (*schema.ProjectGraph, error) {
	if currentProjID == "" {
		return nil, errors.New("no active project")
	}
	return repository.GetGraphData(ctx, currentProjID)
}
...
func GetProjectStatus(ctx context.Context, projectID string) ([]string, []string, error) {
	return repository.GetProjectStatus(ctx, projectID)
}
...
func GetProjectGraph(ctx context.Context, projectID string) (*schema.ProjectGraph, error) {
	return repository.GetGraphData(ctx, projectID)
}
