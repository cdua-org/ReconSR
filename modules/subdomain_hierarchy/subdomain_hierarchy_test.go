...
func TestGetCapabilities(t *testing.T) {
	...
	if len(caps.Functions) == 0 || caps.Functions[0] != "decompose" {
		t.Errorf("expected decompose function, got %v", caps.Functions)
	}
	if len(caps.InputTypes) == 0 || caps.InputTypes[0] != "subdomain" {
		t.Errorf("expected subdomain input type, got %v", caps.InputTypes)
	}
}
