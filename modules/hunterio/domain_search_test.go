...
func TestGetDomainSearch_B2BFullProfile(t *testing.T) {
	...
	m := &module{apiKey: testAPIKey}
	exec := m.getDomainSearch(context.Background(), constants.TypeOrganization, "Enterprise B2B Corporation")
...
func TestGetDomainSearch_DemoKey(t *testing.T) {
	m, ok := New().(*module)
	if !ok {
		t.Fatal("expected module to be *module")
	}
	m.apiKey = "demo-api-key"
	exec := m.getDomainSearch(context.Background(), constants.TypeDomain, "example.com")
	if exec.Error != nil {
...
func TestGetDomainSearch_ShouldBreak(t *testing.T) {
	...
	exec := m.getDomainSearch(context.Background(), constants.TypeDomain, "example.com")
	if exec.Error == nil {
		t.Errorf("expected error on shouldBreak")
	}
}

func TestGetDomainSearch_UnmarshalError(t *testing.T) {
	...
	server.Config.Handler = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		if _, err := w.Write([]byte("{bad_json")); err != nil {
			t.Logf("write failed: %v", err)
		}
	})
	...
	m := &module{apiKey: testAPIKey}
	exec := m.getDomainSearch(context.Background(), constants.TypeDomain, "example.com")
	if exec.Error == nil || !strings.Contains(*exec.Error, "json parse error") {
		t.Errorf("expected json parse error, got: %v", exec.Error)
	}
}

func TestFetchDomainPage_BuildURLError(t *testing.T) {
	origURL := hunterioAPIBaseURL
	defer func() { hunterioAPIBaseURL = origURL }()
	hunterioAPIBaseURL = "://invalid"
	...
	if !shouldBreak {
		t.Errorf("expected shouldBreak to be true on buildURL error")
	}
	if exec.Error == nil || !strings.Contains(*exec.Error, "parse url") {
		t.Errorf("expected 'parse url' error, got: %v", exec.Error)
	}
}
