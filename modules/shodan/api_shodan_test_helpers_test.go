...
func requireTagPropertyResults(t *testing.T, results []schema.ModuleResult, expectedTags ...string) {
	...
	for _, result := range results {
		if len(result.Tags) > 0 {
			hasOnlyAllowedTag := len(result.Tags) == 1 && (result.Tags[0] == constants.TagMX || result.Tags[0] == constants.TagSRV || result.Tags[0] == constants.TagNAPTR || result.Tags[0] == constants.TagCNAME)
