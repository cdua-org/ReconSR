...
func TestModuleCapabilities(t *testing.T) {
	...
	expectedFuncs := []string{"get_domainkey"}
...
func TestGetDomainKeyDataEmpty(t *testing.T) {
	execution := getDomainKeyData("nonexistent.domain.invalid")
	...
	if result.Type != "string" || result.Value != "No DomainKey" {
		t.Errorf("expected 'No DomainKey' result, got %+v", result)
	}
	if result.Context != "DomainKey Records" {
		t.Errorf("expected context 'DomainKey Records', got %q", result.Context)
	}
}
