...
func TestGetSRVDataEmpty(t *testing.T) {
	execution := getSRVData("nonexistent.domain.invalid")
...
func TestSRVCapabilities(t *testing.T) {
	...
	// We haven't updated dns.go yet, but this will pass once we do.
	// Actually, wait, the test runs inside the same package so we can verify if it's there
