...
func TestGetDKIMDataEmpty(t *testing.T) {
	...
	if result.Type != "string" || result.Value != "No DKIM" {
		t.Errorf("expected 'No DKIM' result, got %+v", result)
	}
	if result.Context != "DKIM Records" {
		t.Errorf("expected context 'DKIM Records', got %q", result.Context)
	}
}
